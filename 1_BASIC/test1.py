...
canvas.create_text(480, 120, text='Next Block', font=titleFont, fill='magenta')
...
scoreFont = ('微软雅黑', 24, 'bold')
canvas.create_text(480, 20, text="SCORES: %s" % score, font=scoreFont, fill='magenta')
...
def draw_block(canvas, block):
    ...
    c, r = block['cr']
    cell_list = block['cell_list']
    draw_cells(canvas, c, r, cell_list, SHAPESCOLOR[shape_type])

# 清除下一个形状的背景
def clear_block(canvas):
    color = "#EEEEEE"
    canvas.create_rectangle(380, 160, 580, 320,  fill=color,
...
def check_and_clear():
    ...
    if has_complete_row:
        ...
        canvas.create_rectangle(380, 0, 580, 60,  fill="#EEEEEE",
                            outline="#eeeeee", width=1)
        scoreFont = ('微软雅黑', 24, 'bold')
        canvas.create_text(480, 20, text="SCORES: %s" % score, font=scoreFont, fill='magenta')
...
def game_loop():
    ...
    if current_block is None:
        ...
        draw_block(canvas, next_block)

        next_block['cr'] = [C // 2, 0]
        draw_block_move(canvas, current_block)
